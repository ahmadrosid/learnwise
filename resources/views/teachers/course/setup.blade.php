    ...
    <div class="p-5">
        ...
        <div class="d-flex justify-content-between">
            ...
            <div class="gap-1 d-flex">
                <div class="btn">
                    <a class="link-primary" href="{{ route('course.show', $course) }}">Vew course</a>
                </div>
                <form action="{{ route('teacher.course.publish', $course->id) }}" method="POST">
                    @csrf
                    @method('put')
                    <input type="hidden" value="{{ $course->id }}" name="id" />
                    <button class="btn btn-primary" type="submit" {{ !$isReadyToPublish || $hasBeenSold ? 'disabled' : '' }}>
                ...
                <form action="{{ route('teacher.course.delete', $course->slug) }}" method="POST">
                    ...
                    <button {{ $hasBeenSold ? 'disabled' : '' }} type="submit" class="btn btn-outline-danger" title="Delete course">
        ...
        <div>
            <div class="flex-wrap gap-2 py-2 d-flex">
                ...
                <div class="p-1 bg-light fs-sm d-flex gap-1 align-items-center {{ $field['value'] ? 'text-success' : 'text-danger' }}">
                    @if ($field['value'])
        ...
        <div class="py-5 row row-cols-sm-1 row-cols-md-1 row-cols-lg-2 g-5">
            <div class="col">
                <div class="gap-4 py-2 d-flex align-items-center">
                    <div class="bg-blue-50 d-flex justify-content-center rounded-circle" style="width: 35px; height: 35px; padding: 6px;">
                    ...
                    <div class="p-2 px-3 border rounded-2 bg-neutral-30 border-neutral-40">
                        ...
                        <div class="py-2" x-show="open">
                            <form action="{{ route('teacher.course.update', $course->slug) }}" method="POST">
                                ...
                                @method('put')
                                <div class="py-2">
                                    <select class="border form-select select-choice" aria-label="Select category" name="lang">
                                        <option value="">Select language</option>
                                        @foreach ($languages as $language)
                                        <option value="{{ $language }}" {{ $course->lang === $language ? 'selected' : '' }}>
                                            {{ $language }}
                                        </option>
                                        @endforeach
                    ...
                    <div class="p-2 px-3 border rounded-2 bg-neutral-30 border-neutral-40">
                        ...
                        <div class="py-2" x-show="open">
                            <form action="{{ route('teacher.course.update', $course->slug) }}" method="POST">
                                ...
                                @method('put')
                                <div class="py-2">
                                    <select class="border form-select select-choice" aria-label="Select category" name="category_id">
                    ...
                    <div class="p-2 px-3 border rounded-2 bg-neutral-30 border-neutral-40">
                        <div class="pt-1 pb-2 d-flex justify-content-between align-items-center">
                            ...
                            <button class="gap-2 p-1 btn btn-sm d-flex align-items-center" x-on:click="open = ! open">
                                <x-lucide-plus-circle class="w-3 h-3 cursor-pointer text-neutral-400" x-show="!open" />
                                <span x-show="!open">Add new section</span>
                                <span x-show="open">Cancel</span>
                            </button>
                        </div>
                        <div>
                            <div class="py-2" x-data="{ items: {{ json_encode($sections) }}, newItem: '', dragging: null, dropping: null }" @drop.prevent="items=dragDropList(items, dragging, dropping)" @dragover.prevent="$event.dataTransfer.dropEffect = &quot;move&quot;">
                                ...
                                <div class="py-2" x-data="{ items: {{ json_encode($sections) }}, newItem: '', dragging: null, dropping: null }">
                                    <div class="overflow-hidden border list-group rounded-2">
                                        <template x-for="(item, index) in items" :key="index">
                                            <div class="p-0 position-relative list-group-item border-bottom" :class="{ 'border-bottom-0': items.length - 1 === index }">
                                                    ...
                                                    <template x-if="editMode">
                                                        ...
                                                        <div class="flex-grow-1">
                                                            <form action="/teacher/section/update" method="POST">
                                                                @csrf
                                                                @method('put')
                                                                <input type="text" class="p-0 border-transparent form-control ps-2" x-bind:value="item.title" name="title" />

                                                                <input type="hidden" name="course_id" value="{{ $course->id }}" />
                                                                <input type="hidden" name="section_id" x-bind:value="item.id" />
                                                            </form>
                                                        </div>
                                                    </template>
                                                    <template x-if="!editMode">
                                                        <span x-text="item.title" class="flex-grow-1 ps-2" x-show="!editMode"></span>
                                                    </template>
                                                    <div class="gap-2 d-flex align-items-center">

                                                        <button type="button" class="px-1 text-center btn">
                                                            <x-lucide-pencil class="w-3 h-3" style="margin-right: 8px;" @click="editMode = !editMode" />
                                                        </button>
                                                        <form action="/teacher/section/delete" method="POST">
                                                            ...
                                                            @method('delete')
                                                            <input type="hidden" name="section_id" x-bind:value="item.id" />
                                                            <button type="submit" class="px-1 text-center btn" aria-label="Delete">
                                                                <x-lucide-trash class="w-3 h-3 cursor-pointer text-neutral-400" />
                                                            </button>
                                                        </form>
                                                    </div>
                                                </div>
                                    ...
                                    <div class="pb-2" x-show="open" x-trap="open">
                                        <form action="{{ route('section.create') }}" method="POST">
                                            @csrf
                                            <div class="py-2 input-group">
                                                <input value="" x-model="newItem" type="text" class="form-control" name="section_title" />
                                            </div>
                                            <input type="hidden" name="course_id" value="{{ $course->id }}" />
                                            <button type="submit" class="btn btn-primary">Save</button>
                                        </form>
                            ...
                            <div class="py-2" x-data="{ items: {{ json_encode($chapters) }}, newItem: '', dragging: null, dropping: null }" @drop.prevent="items=dragDropList(items, dragging, dropping)" @dragover.prevent="$event.dataTransfer.dropEffect = &quot;move&quot;">
                                ...
                                <div class="py-2" x-data="{ items: {{ json_encode($chapters) }}, newItem: '', dragging: null, dropping: null }" @drop.prevent="items=dragDropList(items, dragging, dropping)" @dragover.prevent="$event.dataTransfer.dropEffect = &quot;move&quot;">
                                    <div class="overflow-hidden border border-blue-100 list-group rounded-2">
                                        <template x-for="(item, index) in items" :key="index">
                                            <div class="p-0 border-blue-100 position-relative list-group-item border-bottom" draggable="true" :class="{ 'border-bottom-0': items.length - 1 === index }" @dragstart="dragging = index" @dragend="dragging = null">
                                                <div>
                                                    <button class="py-2 px-2 border-0 border-blue-100 btn rounded-0 border-end cursor-grab">
                                                        <x-lucide-grip-vertical class="w-5 h-5 cursor-pointer text-neutral-400" />
                                                    </button>
                                                    ...
                                                    <div class="px-2 pt-1 float-end d-flex align-items-center">
                                                        <template x-if="item.is_published">
                                                            <span class="px-1 text-white rounded bg-success" style="font-size:small;">Published</span>
                                                        </template>

                                                        <template x-if="!item.is_published">
                                                            <span class="px-1 text-white rounded bg-dark" style="font-size:small;">Draft</span>
                                                        </template>
                                                ...
                                                <div class="position-absolute" style="top: 0; bottom: 0; right: 0; left: 0;" x-show.transition="dragging !== null" :class="{
                                                        'bg-blue-100': dropping === index,
                                                        'cursor-grabbing': dragging ===
                                                            index
                                                    }" @dragenter.prevent="if(index !== dragging) {dropping = index}" @dragleave="if(dropping === index) dropping = null"></div>
                                            </div>
                                        </template>
                                    </div>
                                    <div class="pb-2" x-show="open" x-trap="open">
