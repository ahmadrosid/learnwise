                ...
                <ul class="menu-list">
                    ...
                    <li>
                        <div class="p-4 menu-item">
                            <button class="p-0 bg-transparent accordion-button text-reset {{ $activeSession === $section->id ? '' : ' collapsed' }}" type="button" data-bs-toggle="collapse" aria-expanded="false" data-bs-target="#{{ 'section_' . $section->id }}">
                                <x-lucide-bookmark-check class="w-4 h-4 me-2" /> {{ $section->title }}
                            </button>
                        </div>
                        <div class="accordion-collapse collapse {{ $activeSession === $section->id ? ' show' : '' }}" id="{{ 'section_' . $section->id }}">
                            <ul class="menu-list">
                                @if ($section->chapters->count() > 0)
                                @foreach ($chapters as $item)
                                @if ($item->section_id === $section->id)
                                <li>
                                    <a class="menu-item py-4 {{ $item->position == $chapterPosition ? 'active rounded-0 border-blue-100 border-4 border-end' : '' }}" href="/course/{{ $slug }}/chapter/{{ $item->position }}">
    ...
    <main class="bg-neutral-10">
        <div class="p-5">
            <div class="pb-2">
                ...
                <video src="{{ asset('/storage/' . $chapter->video_url) }}" class="rounded card-img-top" controls></video>
